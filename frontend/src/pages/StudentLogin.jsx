...
export default function StudentLogin() {
  ...
  const onSend = async () => {
    const res = await sendOTP(phone, 'student')
    if (res.status === 'ok') { 
      setSent(true)
  ...
  const onVerify = async () => {
    const res = await verifyOTP(phone, otp, 'student')
    if (res.status === 'ok') {
        ...
        <div className="relative rounded-[14px] overflow-hidden min-h-[68vh] flex items-center">
          ...
          <div className="relative z-10 w-full grid lg:grid-cols-2 gap-8 p-6 md:p-12">
            <div className="hidden lg:block">
              <h2 className="text-3xl md:text-5xl font-semibold">Student Portal</h2>
              <p className="text-[#d2d6ff]/80 mt-3 max-w-xl">
                Join live classes, access offline content, participate in discussions and take quizzes.
              </p>
              ...
              <div className="mt-6">
                <p className="text-sm text-[#d2d6ff]/60">
                  Are you a teacher? <a href="/login/teacher" className="text-accent hover:underline">Login here</a>
                </p>
              </div>
            </div>
            ...
            <div className="glass p-6 md:p-8 max-w-md w-full justify-self-end">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-8 h-8 bg-blue-500/20 rounded-lg flex items-center justify-center">
                  <span className="text-blue-400 text-sm font-semibold">S</span>
                </div>
                <h3 className="text-xl font-semibold">Student Login</h3>
              </div>
